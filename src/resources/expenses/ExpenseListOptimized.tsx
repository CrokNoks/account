...
const CategorizeButton = React.memo(() => {
  ...
  return (
    <>
      ...
      {open && <CategorizationModal open={open} onClose={handleClose} />}
    </>
  );
});
...
const ReconciledToggle = React.memo(({ onSuccess, readOnly = false }: { onSuccess?: () => void, readOnly?: boolean }) => {
  ...
  const { mutate: toggleReconciled } = useOptimisticExpenseUpdate();
...
export const ExpenseListOptimized = React.memo(({ filter, embed = false, actions = <></>, onRowClick, onUpdate, readOnly = false }: { filter: any, embed?: boolean, actions?: any, onRowClick?: (id: any) => false, onUpdate?: () => void, readOnly?: boolean }) => {
  const { selectedAccountId } = useAccount();
  const isSmall = useIsSmall();

  // Memoize filters and actions to prevent recreation
  const memoFilters = useMemo(() => expenseFilters(selectedAccountId, embed), [selectedAccountId, embed]);
  const memoActions = useMemo(() => embed ? actions : <ExpenseListActions />, [embed, actions]);
  ...
  return (
    <List
      ...
      queryOptions={{
        staleTime: 1000 * 30, // 30 seconds for list data
    ...
    >
      {isSmall ? (
        <SimpleList
          ...
          secondaryText={(record) => formatDate(record.date)}
          tertiaryText={(record) => formatCurrency(record.amount)}
        ...
        <Datagrid {...(embed ? { bulkActionButtons: false, rowClick: onRowClick || false } : { rowClick: 'show' })}>
          ...
          {embed && readOnly ? null : embed ? null : <EditButton />}
          {embed && readOnly ? null : embed ? null : <DeleteButton />}
        </Datagrid>)}
    </List>
  );
});
...
export const ExpenseList = ExpenseListOptimized;