        ...
        const fetchData = async () => {
            ...
            try {
              ...
              setLastReport(lastReport);
                ...
                setCategoryData(Array.from(categoryMap.values()).sort((a, b) => b.totalExpense - a.totalExpense));
                            ...
                            <Grid container spacing={2}>
                                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                                    <Typography color="text.secondary" variant="body2">
                                        Nombre de Rapports du {firstReport ? new Date(firstReport).toLocaleDateString('fr-FR', { month: 'short', year: '2-digit' }) : ''} au {lastReport ? new Date(lastReport).toLocaleDateString('fr-FR', { month: 'short', year: '2-digit' }) : ''}
                                    </Typography>
                                    ...
                                    {categoryData.map((cat) => (
                                        <Box key={cat.categoryId} mb={2}>
                                            ...
                                            <Typography variant="body2" color="text.secondary">
                                                Total: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(cat.totalExpense)}
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary">
                                                Min: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(cat.min)} /
                                                Max: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(cat.max)} /
                                                Moyenne: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(cat.totalExpense / cat.reportCount)}  
                                            </Typography>
                                            <Typography variant="caption" color="text.secondary">
                                                Sur {cat.reportCount} rapport(s)
                                            </Typography>
                                        </Box>
                                    ))}
                                </Box>
