...
import { Period } from '../../periods/domain/period.entity';
...
describe('GetAnomaliesUseCase', () => {
  ...
  it('should detect duplicate transactions on the same day', async () => {
    ...
    transactionRepository.findAllByAccount.mockResolvedValue([tx1, tx2, txNormal]);
  ...
  it('should ignore small amounts (<5€) and incomes for duplicates', async () => {
    const txSmall1 = Transaction.create({ id: 'tx-1', accountId, categoryId: 'cat-1', date: new Date(2026, 5, 10), description: 'Baguette', amount: BigInt(-120), metadata: {} });
    const txSmall2 = Transaction.create({ id: 'tx-2', accountId, categoryId: 'cat-1', date: new Date(2026, 5, 10), description: 'Baguette', amount: BigInt(-120), metadata: {} });
    
    const txIncome1 = Transaction.create({ id: 'tx-3', accountId, categoryId: 'cat-2', date: new Date(2026, 5, 10), description: 'Virement', amount: BigInt(5000), metadata: {} });
    const txIncome2 = Transaction.create({ id: 'tx-4', accountId, categoryId: 'cat-2', date: new Date(2026, 5, 10), description: 'Virement', amount: BigInt(5000), metadata: {} });

    transactionRepository.findAllByAccount.mockResolvedValue([txSmall1, txSmall2, txIncome1, txIncome2]);
  ...
  it('should detect outlier transactions (> 3x average)', async () => {
    ...
    const history = [
      Transaction.create({ id: 'h1', accountId, categoryId: 'cat-1', date: new Date(2026, 2, 1), description: 'Test', amount: BigInt(-1000), metadata: {} }),
      Transaction.create({ id: 'h2', accountId, categoryId: 'cat-1', date: new Date(2026, 3, 1), description: 'Test', amount: BigInt(-1000), metadata: {} }),
      Transaction.create({ id: 'h3', accountId, categoryId: 'cat-1', date: new Date(2026, 4, 1), description: 'Test', amount: BigInt(-1000), metadata: {} }),
    ];
    ...
    transactionRepository.findAllByAccount.mockResolvedValue([...history, outlier]);
