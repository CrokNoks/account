...
import { addDays, getDate, getMonth, getYear, isAfter, isBefore, startOfDay } from 'date-fns';
...
export class GetCashflowForecastUseCase {
  ...
  async execute(accountId: string, days: number = 90): Promise<CashflowForecastResponse> {
    const account = await this.accountRepository.findById(accountId);
    if (!account) throw new Error('Account not found');

    // 1. Get current balance (up to today inclusive)
    const tomorrow = startOfDay(addDays(new Date(), 1));
    const sumAll = await this.transactionRepository.sumAmountByAccountBeforeDate(accountId, tomorrow);
    const currentBalance = account.initialBalance + sumAll;

    // 2. Get recurring transactions
    const recurringTxs = await this.recurringRepository.findAllByAccount(accountId);
